...
with st.spinner("Menghitung rute jalan..."):
    path_outline, path_blue, path_blue_link, path_fallback = [], [], [], []
    path_air_line = []
    hub_dists = {hid: [] for hid in active_ids}
    for i, row in df_mapped.reset_index(drop=True).iterrows():
...
conf_vals = df_mapped['confidence'].reset_index(drop=True)
...
cluster_counts = df_mapped.groupby('cluster_id')['lat'].transform('count').reset_index(drop=True)
cluster_norm = (cluster_counts - 1) / max(cluster_counts.max() - 1, 1)
hub_load = df_mapped.groupby('hub_id')['lat'].transform('count').reset_index(drop=True)
hub_norm = (hub_load - 1) / max(hub_load.max() - 1, 1)
if 'processed_at' in df_mapped.columns:
    ts = pd.to_datetime(df_mapped['processed_at'], errors='coerce', utc=True)
    now_utc = pd.Timestamp.now(tz='UTC')
    age_h = ((now_utc - ts).dt.total_seconds().fillna(0).clip(lower=0)) / 3600.0
    recency_norm = 1.0 - (age_h / 168.0).clip(0, 1)
else:
    recency_norm = pd.Series(np.ones(len(df_mapped)), index=df_mapped.index)
# Severity scale lebih dramatis untuk post-disaster
severity_scale = 0.6 + (1.5 * conf_norm) + (1.2 * cluster_norm) + (0.5 * recency_norm.reset_index(drop=True))
radius_vals = np.clip((base_r * severity_scale).astype(int), int(base_r * 0.6), int(base_r * 4.0))

red_df = pd.DataFrame({
    'lon': df_mapped['lon'].values,
    'lat': df_mapped['lat'].values,
    'desa': df_mapped[desa_col].values,
    'tingkat_keyakinan': (conf_vals * 100).round(1).astype(str) + '%',
    'radius': radius_vals
})
...
layers += [
    pdk.Layer("ScatterplotLayer", red_df,
              ...
              get_fill_color=[220, 38, 38, 210], get_line_color=[254, 202, 202],
              line_width_min_pixels=1, radius_scale=1, pickable=True),
    pdk.Layer("ScatterplotLayer",
              ...
              get_fill_color=[22, 163, 74, 240], get_line_color=[134, 239, 172],
