...
BASE = os.path.normpath(os.path.join(os.path.dirname(__file__), "..", "backend", "output"))
...
def root():
    ...
    response = {
        "disaster_info": {"types": list({r["disaster_type"] for r in red_zones}), "summary": ""},
        "metrics": {"active_areas": max(0, len({r["desa"] for r in red_zones})), "total_damage": len(red_zones), "estimated_impacts": len(red_zones)*4},
        "total_logistics": {},
